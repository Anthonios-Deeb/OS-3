...
bool kosaraju_has_majority = false;
bool kosaraju_nolonger_majority = false;

void signal_kosaraju_has_majority()
{
    kosaraju_has_majority = true;
    pthread_cond_signal(&cond);
}

void signal_kosaraju_no_longer_majority()
{
    kosaraju_nolonger_majority = true;
    pthread_cond_signal(&cond);
}
...
void processClientRequest(op_Packet p, int client_socket)
{
    ...
    else if (p.operation == "kosaraju")
    {
        ...
        {
            graph->findSCC();
            vector<list<int>> scc = graph->getScc();
            if (!graph->getHasMajority())
            {
                if (graph->calcIfHasMajority())
                {
                    signal_kosaraju_has_majority();
                }
            }
            else if (graph->getHasMajority())
            {
                if (!graph->calcIfHasMajority())
                {
                    signal_kosaraju_no_longer_majority();
                }
            }
...
// thread function to notify changes in graph scc
void *notifyThreadFunc(void *arg)
{
    ...
    while (true)
    {
        while (!kosaraju_has_majority && !kosaraju_nolonger_majority)
        {
            pthread_cond_wait(&cond, &clientMutex);
        }
        if (kosaraju_has_majority)
        {
            cout << "At least 50% of the graph belongs to the same SCC" << endl;
            kosaraju_has_majority = false; // Reset the flag
        }
        else if (kosaraju_nolonger_majority)
        {
            cout << "At Least 50% of the graph no longer belongs to the same SCC" << endl;
            kosaraju_nolonger_majority = false; // Reset the flag
        }
    }
...
int main()
{
    ...
    // Create the notifyThread to handle notifications
    pthread_t notifyThread;
    if (pthread_create(&notifyThread, NULL, notifyThreadFunc, NULL) != 0)
    {
        perror("Failed to create notifyThread");
        exit(EXIT_FAILURE);
    }
    ...
    // Join and cleanup the notifyThread
